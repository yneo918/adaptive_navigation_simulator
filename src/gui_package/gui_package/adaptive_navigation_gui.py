...
class AdaptiveNavigationGUI(Node):    
    def __init__(self):
        super().__init__('adaptive_navigation_gui')
        ...
        self.cluster_d2: float = 10.0
        self.cluster_d3: float = 10.0
        self.cluster_d4: float = 10.0
        self.cluster_d5: float = 10.0
    ...
    def _setup_subscriptions(self) -> None:
        ...
        self.pubsub.create_subscription(
            Float32MultiArray, '/ctrl/cluster_desired', 
    ...
    def _process_mode_changes(self, msg, toggle) -> None:
        ...
        if (self.angle_sel_button and toggle[self.angle_sel_button] == 1 and
            ...
            self.pubsub.publish('/ctrl/cluster_params', reset_msg)
    ...
    def timer_callback(self):
        ...
        self.pubsub.publish('/ctrl/select_rover', select_msg)
        
        # Publish operation mode
        mode_msg = String()
        mode_msg.data = self.rover_mode.value
        self.pubsub.publish('/ctrl/cluster_mode', mode_msg)
        ...
        self.pubsub.publish('/ctrl/adaptive_mode', adaptive_nav_mode_msg)

        # Publish enable
        enable_msg = Bool()
        enable_msg.data = self.enable
        self.pubsub.publish('/ctrl/enable', enable_msg)
...
class StatusWindow5Robot(StatusWindowBase):
    ...
    def __init__(self, node: AdaptiveNavigationGUI):
    ...
    def _add_custom_controls(self, layout: QVBoxLayout) -> None:
        ...
        for i, param_name in enumerate(["b3", "b4", "b5"], 3):
            spin = QDoubleSpinBox()
            spin.setMinimum(-10.0 * math.pi)
            spin.setMaximum(10.0 * math.pi)
            spin.setValue(getattr(self.node, f"cluster_{param_name}"))
            spin.setSingleStep(0.1)
            spin.valueChanged.connect(
    ...
    def _publish_cluster_params(self) -> None:
        ...
        self.node.pubsub.publish('/ctrl/cluster_params', msg)
