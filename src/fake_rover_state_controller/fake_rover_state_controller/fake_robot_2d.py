...
class FakeRover(Node):
    ...
    def __init__(self):
        ...
        self.vel = {'x': 0.0, 'y': 0.0, 'rotate': 0.0, 'alive': 0}
    ...
    def teleop_callback(self, msg):
        self.vel.update({'x': max(-MAX_TRANS, min(msg.linear.x, MAX_TRANS)), 'y': max(-MAX_TRANS, min(msg.linear.y, MAX_TRANS)), 'rotate': max(-MAX_ROTATE, min(msg.angular.z, MAX_ROTATE)), 'alive': VEL_ALIVE})
