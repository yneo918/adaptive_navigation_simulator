...
MAX_SENSOR = -10.0  # Max expected sensor value in dBm
MIN_SENSOR = -105.0  # Min expected sensor value in dBm
...
class ANNode(Node):
    ...
    def normalize(self, val, min_val=100.0, max_val=0.0):
    ...
    def publish_velocities(self):
        ...
        if self.num_robots == 5:
            dz1 = self.gradient.robot_positions[1][2] - self.gradient.robot_positions[3][2]
            dz2 = self.gradient.robot_positions[2][2] - self.gradient.robot_positions[4][2]
            ...
            sgnx = self.gradient.mode.gain * self.gradient.sign(dz3+dz4)
            sgny = self.gradient.mode.gain * self.gradient.sign(dz1+dz2)
            sgnt = self.gradient.mode.gain * self.gradient.sign(dz3-dz4)
            _msg.linear.x = sgnx * KV
            _msg.linear.y = sgny * KV
            _msg.angular.z = sgnt * KV
